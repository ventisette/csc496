...
class hero:
    ...
    def __init__(self, width = 0, height = 0, health = 0, mana = 0, x = 0, y = 0, img = (0, 0, 0)):
        ...
        self.X = x
        self.Y = y
        self.MOVESPEEDX = 1.25
        self.MOVESPEEDY = 1.25
        self.MOVESPEEDDIAG = (self.MOVESPEEDX / math.sqrt((self.MOVESPEEDX*1.7)**2) )* self.MOVESPEEDX * (math.sqrt((self.MOVESPEEDX**2) + (self.MOVESPEEDY**2))/1.4)
        self.set_IMG(img)
    ...
    def move(self, keys_pressed, screen_size):
        ...
        if(keys_pressed[119] == 1 and keys_pressed[97] == 1):    #w and a
            ...
            catchEmAll = False 
        if(keys_pressed[119] == 1 and keys_pressed[100] == 1): #w and d                           
            self.X += self.MOVESPEEDDIAG
            self.Y -= self.MOVESPEEDDIAG
            catchEmAll = False
        if(keys_pressed[115] == 1 and keys_pressed[97] == 1):  #s and a  
            self.X -= self.MOVESPEEDDIAG
            self.Y += self.MOVESPEEDDIAG
            catchEmAll = False
        if(keys_pressed[115] == 1 and keys_pressed[100] == 1): #s and d
            self.X += self.MOVESPEEDDIAG
            self.Y += self.MOVESPEEDDIAG
            catchEmAll = False
        if(catchEmAll):
            ...
            if(keys_pressed[100] == 1):   #d
                self.X += self.MOVESPEEDX
        print(str(keys_pressed[119]) + " " + str(keys_pressed[100]) + " "+str(keys_pressed[97]) + " "+str(keys_pressed[115]))
        """
