...
class cMenu:
      ...
      try:
         ...
         # based on the image provided
            ...
            if button_info[0] != None:
               r = self.font.render
               text_image   = r(button_info[0], True, self.s_color)
               text_rect = text_image.get_rect()
               width, height = text_rect.size
            else:
               width, height = img_btn.get_size()
            ...
            try:
               ...
               print e
               if button_info[0] != None:
                  if img_btn.get_width()*img_btn.get_height() > text_rect.width * text_rect.height:
                     w, h = text_rect.size
                     img_btn = pygame.transform.smoothscale(img_btn, (w, h))
                  else:
                     img_btn = pygame.transform.smoothscale(img_btn, (img_btn.get_width(), img_btn.get_height()))
   ...
   def set_button_images(self):
      for button in self.menu_items:
         ...
         if button['b_image'] != None and button['text'] != None:
            r = self.font.render
            width  = button['rect'][2]
            ...
            x = selected_rect.center[0] - button['b_image'].get_width()/2
            y = selected_rect.center[1] - button['b_image'].get_height()/2
            selected_image.blit(self.background, (0, 0), rect)
            selected_image.blit(button['b_image'], (x, y))
            text_image   = r(button['text'], True, self.s_color)
            text_rect = text_image.get_rect()
            x = selected_rect.center[0] - text_rect.width/2
            y = selected_rect.center[1] - text_rect.height/2
            selected_image.blit(text_image, (x, y))
            
            unselected_image = pygame.Surface((width, height), -1)
            unselected_rect = unselected_image.get_rect()
            x = unselected_rect.center[0] - button['b_image'].get_width()/2
            y = unselected_rect.center[1] - button['b_image'].get_height()/2
            unselected_image.blit(self.background, (0, 0), rect)
            unselected_image.blit(button['b_image'], (x, y))
            text_image   = r(button['text'], True, self.u_color)
            text_rect = text_image.get_rect()
            x = unselected_rect.center[0] - text_rect.width/2
            y = unselected_rect.center[1] - text_rect.height/2
            unselected_image.blit(text_image, (x, y))
         #------------------------------------------------------------------------------------------
         elif button['b_image'] == None:
            r = self.font.render
            width  = button['rect'][2]
            height = button['rect'][3]
            rect = pygame.Rect(button['offset'], (width, height))
