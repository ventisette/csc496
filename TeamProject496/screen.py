...
#draw########################################################################      
    def draw(self, source_pos, destination_pos = None):
        '''Name: draw
        ...
        if isinstance(source_pos, tuple):
            source_pos, destination_pos = source_pos
...
#update_explicit######################################################################## 
    ...
    """
#draw_background######################################################################## 
    def draw_background(self, location = (0,0)):
        ...
        if isinstance(self.ACTUAL_BACK_IMG, (list, tuple)):
            ...
            self.SCREEN.blit(self.SCREEN_BACK_IMG, location )#copies the background image created earlier to the screen
            
#write######################################################################## 
    def write(self, text, location, color = (255, 255, 255), ftype = 0):
        '''Name: write
           ...
           Returns: None
        '''
        source = self.FONTS[ftype].render(str(text), 1, color)
...
#set_font#######################################################################
    def set_font(self, size = 12, Filename=None):
        '''Name: set_font
        ...
        if isinstance(size, tuple):
            for sizeAndType in size:
                x = pygame.font.Font(sizeAndType[1], sizeAndType[0])
                self.FONTS.append(x)
        else:
